...
int main() {
    ...
    // המרה למצביעים מסוג __m256i*
    __m256i* vec_input  = reinterpret_cast<__m256i*>(input);
    __m256i* vec_output = reinterpret_cast<__m256i*>(output);

    // חישוב SIMD: כל פעם 8 איברים
    for (int i = 0; i < num_elements / 8; ++i) {
        __m256i vec     = _mm256_load_si256(&vec_input[i]);
        __m256i squared = _mm256_mullo_epi32(vec, vec);
        _mm256_store_si256(&vec_output[i], squared);
    }
